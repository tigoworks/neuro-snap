    ...
    async submitAnswers(req, res) {
        ...
        try {
            ...
            if (missingTests.length > 0) {
                ...
                logger_1.default.warn('❌ 答案提交失败：数据不完整', {
                    missingFields: Object.fromEntries(Object.entries(testTypeInfo).map(([key, info]) => [key, !info.present])),
                    ...
                    inputParams: {
                        originalRequest: {
                            ...
                            testAnswerCounts: Object.fromEntries(Object.entries(testTypeInfo).map(([key, _]) => [
                                key,
                ...
                logger_1.default.error('🎯 前端处理指南：缺失字段详情', {
            ...
            // 开始事务日志
            logger_1.default.info('🔄 开始答案提交事务', {
                transactionId,
                userInfo: {
            ...
            database_logger_service_1.databaseLogger.logQuerySuccess(modelsQueryId, modelsData, Date.now(), {
                table: 'survey_model',
                operation: 'SELECT'
            });
            // 3. 获取所有题目信息
            const modelIds = modelsData.map((model) => model.id);
            const questionsQueryId = database_logger_service_1.databaseLogger.logQueryStart({
            ...
            if (questionsError || !questionsData) {
                database_logger_service_1.databaseLogger.logQueryError(questionsQueryId, questionsError, Date.now(), {
                    table: 'survey_question',
                    operation: 'SELECT'
                });
                database_logger_service_1.databaseLogger.logTransaction('rollback', transactionId);
            ...
            database_logger_service_1.databaseLogger.logQuerySuccess(questionsQueryId, questionsData, Date.now(), {
                table: 'survey_question',
                operation: 'SELECT'
            });
            // 4. 准备答案数据
            ...
            for (const testType of testTypes) {
                const model = modelsData.find((m) => m.code === testType.code);
                if (model && testType.data) {
                    Object.entries(testType.data).forEach(([questionCode, answer]) => {
                        const question = questionsData.find((q) => q.model_id === model.id && q.question_code === questionCode);
                        if (question) {
                            // 处理排序题的默认值
                            ...
                            answerStats[testType.code]++;
                        }
                    });
                }
            }
            logger_1.default.info('📊 答案数据准备完成', {
                transactionId,
            ...
            // 记录事务失败
            database_logger_service_1.databaseLogger.logTransaction('rollback', transactionId);
            logger_1.default.error('❌ 答案提交事务失败', {
                transactionId,
                error: {
