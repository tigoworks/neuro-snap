...
exports.MCPKnowledgeService = void 0;
...
class MCPKnowledgeService {
    constructor() {
        ...
        this.knowledgeBasePath = path_1.default.join(process.cwd(), 'knowledge-base');
    }
    /**
     * 初始化知识库
     */
    async initialize() {
        ...
        try {
            // 创建知识库目录
            await promises_1.default.mkdir(this.knowledgeBasePath, { recursive: true });
            // 创建子目录
            const subDirs = ['files', 'documents', 'analysis', 'reports'];
            for (const dir of subDirs) {
                await promises_1.default.mkdir(path_1.default.join(this.knowledgeBasePath, dir), { recursive: true });
            }
            // 确保数据库表存在
     ...
     * 导入文件到知识库
     */
    async importFile(filePath, category = 'general', tags = []) {
        const queryId = await this.logger.logQuery({
        ...
        try {
            ...
            // 复制文件到知识库目录
            const targetPath = path_1.default.join(this.knowledgeBasePath, 'files', fileName);
     ...
     * 从网站抓取内容
     */
    async importFromUrl(url, category = 'web', tags = []) {
        const queryId = await this.logger.logQuery({
     ...
     * 获取相关知识内容（用于AI分析）
     */
    async getRelevantKnowledge(userAnswers) {
        const queryId = await this.logger.logQuery({
        ...
        try {
            ...
            // 去重
            const uniqueEntries = relevantEntries.filter((entry, index, self) => index === self.findIndex(e => e.id === entry.id));
            await this.logger.logQueryResult(queryId, {
                success: true,
     ...
     * 从用户答案中提取关键词
     */
    extractKeywords(userAnswers) {
        const keywords = [];
        // 从五问法中提取关键词
        if (userAnswers.fiveQuestions) {
            Object.values(userAnswers.fiveQuestions).forEach((answer) => {
        ...
        // 添加测试类型相关关键词
        if (userAnswers.mbti)
            keywords.push('MBTI', 'personality', 'myers-briggs');
        if (userAnswers.bigFive)
            keywords.push('big five', 'personality traits', 'ocean');
        if (userAnswers.disc)
            keywords.push('DISC', 'behavior', 'communication style');
        if (userAnswers.holland)
            keywords.push('Holland', 'career', 'interests', 'RIASEC');
        if (userAnswers.values)
            keywords.push('values', 'motivation', 'work values');
