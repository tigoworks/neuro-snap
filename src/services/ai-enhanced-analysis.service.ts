...
import { AutoAnalysisService, AnalysisRequest, AnalysisResult } from './auto-analysis.service';
...
export class AIEnhancedAnalysisService {
  ...
  private initializeOpenAI(): void {
    ...
    if (!apiKey) {
      logger.warn('⚠️  OpenAI API密钥未配置，将使用规则分析作为降级方案');
      return;
    }
    ...
    try {
      ...
      // 如果配置了代理，使用SOCKS5代理
      if (proxyUrl) {
        logger.info('🔧 使用SOCKS5代理配置OpenAI客户端', { proxyUrl });
    ...
    try {
      ...
      if (this.openai) {
        try {
          ...
          logger.info('✅ AI分析成功', { analysisId, userId: request.userId });
        } catch (aiError) {
          logger.warn('⚠️  AI分析失败，降级到规则分析', { 
            analysisId, 
            userId: request.userId,
        ...
        logger.info('📋 使用规则分析（AI未配置）', { analysisId, userId: request.userId });
        analysisResult = await this.performRuleBasedAnalysis(request, knowledgeBase);
      }
   ...
   * 执行AI分析
   */
  private async performAIAnalysis(request: AnalysisRequest, knowledgeBase: any[]): Promise<any> {
    if (!this.openai) {
   ...
   * 构建智能prompt
   */
  private async buildIntelligentPrompt(userInfo: any, answers: any, knowledgeBase: any[]): Promise<string> {
    // 用户基本信息
    const userProfile = `
用户基本信息：
...
`;
   ...
   * 格式化测试答案 - 查询题目内容和选项文本
   */
  private async formatTestAnswers(answers: any): Promise<string> {
    let formatted = '\n测试答案：\n';
        ...
        for (const [questionCode, answer] of Object.entries(answers.fiveQuestions)) {
          ...
          if (question) {
            ...
            if (question.type === 'text') {
              ...
              const selectedOption = question.options.find((opt: any) => opt.code.toString() === answer);
          ...
          if (question) {
            ...
            if (question.type === 'text') {
              ...
              const selectedOptions = answer.map(code => {
                const option = question.options.find((opt: any) => opt.code.toString() === code);
              ...
              if (typeof answer === 'object' && answer && 'order' in answer && Array.isArray(answer.order)) {
                const sortedOptions = answer.order.map((code: any) => {
   ...
   * 格式化知识库内容
   */
  private formatKnowledgeBase(knowledgeBase: any[]): string {
...
请确保返回的JSON格式正确，所有字段都有合理的内容，特别是成长周期和未来成就预测部分要具体、可量化。`;
   ...
   * 增强AI分析结果
   */
  private enhanceAIAnalysis(aiAnalysis: any, answers: any, knowledgeBase: any[]): any {
    // 确保必要字段存在
    const enhanced = {
   ...
   * 执行规则分析（降级方案）
   */
  private async performRuleBasedAnalysis(request: AnalysisRequest, knowledgeBase: any[]): Promise<any> {
    // 使用现有的规则分析服务
   ...
   * 生成降级推荐
   */
  private generateFallbackRecommendations(answers: any): string[] {
  ...
  async getAIStatus(): Promise<{
    ...
    try {
      ...
      logger.warn('⚠️  OpenAI API健康检查失败', { 
