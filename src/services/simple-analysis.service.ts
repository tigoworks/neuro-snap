...
export class SimpleAnalysisService {
  ...
  private analyzeUserProfile(userInfo: any) {
    return {
      age: userInfo.age || 0,
  ...
  private analyzeMBTI(mbtiData: any) {
    // 简化的MBTI分析逻辑
    ...
    answers.forEach((answer: any, index) => {
      ...
      switch (index % 4) {
        case 0: value > 2 ? scores.E++ : scores.I++; break;
        case 1: value > 2 ? scores.S++ : scores.N++; break;
        case 2: value > 2 ? scores.T++ : scores.F++; break;
        case 3: value > 2 ? scores.J++ : scores.P++; break;
      }
    });
  ...
  private analyzeBigFive(bigFiveData: any) {
    const scores = {
      openness: 50,
    ...
    // 基于答案计算分数（简化逻辑）
    Object.entries(bigFiveData).forEach(([key, value]: [string, any]) => {
      const score = parseInt(value) || 3;
      if (key.includes('o_')) scores.openness += (score - 3) * 10;
      if (key.includes('c_')) scores.conscientiousness += (score - 3) * 10;
      if (key.includes('e_')) scores.extraversion += (score - 3) * 10;
      if (key.includes('a_')) scores.agreeableness += (score - 3) * 10;
      if (key.includes('n_')) scores.neuroticism += (score - 3) * 10;
    });

    // 限制在0-100范围内
    Object.keys(scores).forEach(key => {
      (scores as any)[key] = Math.max(0, Math.min(100, (scores as any)[key]));
    });

    return scores;
  }

  private analyzeDISC(discData: any) {
    const scores = { D: 0, I: 0, S: 0, C: 0 };
    
    Object.entries(discData).forEach(([key, value]: [string, any]) => {
      const score = parseInt(value) || 1;
      if (key.includes('d_')) scores.D += score;
      if (key.includes('i_')) scores.I += score;
      if (key.includes('s_')) scores.S += score;
      if (key.includes('c_')) scores.C += score;
    });
    ...
    const primaryStyle = Object.entries(percentages).reduce((a, b) => 
      (percentages as any)[a[0]] > (percentages as any)[b[0]] ? a : b
    )[0];
  ...
  private analyzeHolland(hollandData: any) {
    const scores = { R: 0, I: 0, A: 0, S: 0, E: 0, C: 0 };
    
    Object.entries(hollandData).forEach(([key, value]: [string, any]) => {
      const score = parseInt(value) || 1;
      if (key.includes('r_')) scores.R += score;
      if (key.includes('i_')) scores.I += score;
      if (key.includes('a_')) scores.A += score;
      if (key.includes('s_')) scores.S += score;
      if (key.includes('e_')) scores.E += score;
      if (key.includes('c_')) scores.C += score;
    });
  ...
  private analyzeValues(valuesData: any) {
    const categories = {
      achievement: 0,
      security: 0,
      relationship: 0,
    ...
    Object.entries(valuesData).forEach(([key, value]: [string, any]) => {
      if (Array.isArray(value)) {
        value.forEach(v => {
          const val = parseInt(v) || 0;
          // 简化的价值观分类逻辑
          if (val <= 2) categories.achievement++;
          else if (val <= 4) categories.security++;
          else if (val <= 6) categories.relationship++;
          else if (val <= 8) categories.autonomy++;
          else categories.service++;
        });
      }
    });
  ...
  private generateInsights(analyses: any) {
  ...
  private getMBTIDescription(type: string): string {
    ...
    return (descriptions as any)[type] || '独特的人格类型';
  }

  private getDISCStyleName(style: string): string {
    const names = {
      dominance: '支配型',
      influence: '影响型',
      steadiness: '稳健型',
    ...
    return (names as any)[style] || '混合型';
  }

  private getCareerRecommendations(mbtiType: string, hollandTypes: string[], discStyle: string): string[] {
    // 简化的职业推荐逻辑
  ...
  private getStrengths(mbtiType: string, bigFive: any, discStyle: string): string[] {
    const strengths = [];
    
    if (bigFive.openness > 60) strengths.push('富有创造力', '乐于接受新事物');
    if (bigFive.conscientiousness > 60) strengths.push('认真负责', '注重细节');
    if (bigFive.extraversion > 60) strengths.push('善于社交', '活力充沛');
    if (bigFive.agreeableness > 60) strengths.push('善于合作', '值得信赖');
    if (bigFive.neuroticism < 40) strengths.push('情绪稳定', '抗压能力强');
    
    return strengths.length > 0 ? strengths : ['独特的个人魅力', '具有发展潜力'];
  }

  private getImprovementAreas(bigFive: any, mbtiType: string): string[] {
    const areas = [];
    
    if (bigFive.extraversion < 40) areas.push('提高社交技能', '增强沟通表达');
    if (bigFive.conscientiousness < 40) areas.push('提升时间管理', '加强执行力');
    if (bigFive.openness < 40) areas.push('培养创新思维', '接受新观念');
    if (bigFive.neuroticism > 60) areas.push('情绪管理', '压力调节');
