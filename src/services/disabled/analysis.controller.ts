...
import { OpenAIAnalysisService, AnalysisRequest } from '../services/openai-analysis.service';
import { MCPKnowledgeService } from '../services/mcp-knowledge.service';
...
export class AnalysisController {
  ...
  async importFile(req: Request, res: Response): Promise<void> {
    try {
      ...
      const entryId = await this.knowledgeService.importFile(
        filePath,
        category || 'general',
        tags || []
      );
   ...
   * 从URL导入内容到知识库
   */
  async importFromUrl(req: Request, res: Response): Promise<void> {
    try {
      ...
      const entryId = await this.knowledgeService.importFromUrl(
        url,
        category || 'web',
        tags || []
      );
