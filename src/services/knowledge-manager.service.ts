...
export interface KnowledgeItem {
  ...
  content: string;
  model_tag: string;
  source_type: 'static' | 'vector' | 'external' | 'company_values';
  category?: string;
  tags?: string[];
  metadata?: any;
  created_at?: string;
...
export class KnowledgeManagerService {
  private static instance: KnowledgeManagerService;
  ...
  public static getInstance(): KnowledgeManagerService {
  ...
  async addKnowledgeItem(item: KnowledgeItem): Promise<string> {
    try {
      logger.info('📚 添加知识条目到知识库', { title: item.title, model_tag: item.model_tag });
      ...
      if (error) {
        logger.error('❌ 添加知识条目失败', { error: error.message });
        throw error;
      }

      logger.info('✅ 知识条目添加成功', { id: data.id, title: item.title });
      return data.id;
    } catch (error: any) {
      logger.error('❌ 知识库操作异常', { error: error.message });
      throw error;
    }
  }
   ...
   * 批量添加企业价值观
   */
  async addCompanyValues(companyName: string, values: CompanyValues[]): Promise<string[]> {
    try {
      logger.info('🏢 批量添加企业价值观', { 
        company: companyName, 
        count: values.length 
      });

      const knowledgeItems: KnowledgeItem[] = values.map(value => ({
   ...
   * 格式化价值观内容
   */
  private formatValueContent(value: CompanyValues): string {
    return `# ${value.title}
...
*此内容为企业价值观知识库条目，用于AI分析时提供文化背景参考*`;
   ...
   * 搜索知识库内容
   */
  async searchKnowledge(query: string, modelTag?: string, limit: number = 10): Promise<KnowledgeItem[]> {
    try {
      logger.info('🔍 搜索知识库', { query, modelTag, limit });
      ...
      logger.error('❌ 知识库搜索异常', { error: error.message });
      throw error;
    }
  }
   ...
   * 获取指定模型的所有知识条目
   */
  async getKnowledgeByModel(modelTag: string): Promise<KnowledgeItem[]> {
    try {
      logger.info('📖 获取模型知识库', { modelTag });
      ...
      logger.error('❌ 获取模型知识库异常', { error: error.message });
      throw error;
    }
  }
  ...
  async deleteKnowledgeItem(id: string): Promise<boolean> {
    try {
      ...
      logger.info('✅ 知识条目删除成功', { id });
      return true;
    } catch (error: any) {
      logger.error('❌ 删除知识条目异常', { error: error.message });
      throw error;
    }
  }
  ...
  async getKnowledgeStats(): Promise<{
    ...
    try {
      ...
      const stats = {
        ...
        byModelTag: {} as Record<string, number>,
      ...
      logger.info('✅ 知识库统计完成', stats);
      return stats;
    } catch (error: any) {
      logger.error('❌ 获取知识库统计异常', { error: error.message });
      throw error;
    }
  }
} 